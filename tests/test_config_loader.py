...
class TestConfigLoader:
    ...
    def test_sweep_config_cache(
        ...
        master_cfg = config_loader.load_configuration(
            config_name="config.yaml",
            overrides=["+time=${now:%H-%M-%S}", "+test_env=${env:TEST_ENV}"],
            run_mode=RunMode.RUN,
        )
        ...
        assert master_cfg.test_env == os.environ["TEST_ENV"]

        sweep_cfg = config_loader.load_sweep_config(
            master_config=master_cfg,
            sweep_overrides=["+time=${now:%H-%M-%S}", "+test_env=${env:TEST_ENV}"],
        )
        ...
        assert sweep_cfg.test_env == os.getenv("TEST_ENV")
